...
Definition sumR_flip {A A' : Type} (AR : A -> A' -> Type)
    {B B' : Type} (BR : B -> B' -> Type) (s : A + B) (s' : A' + B')
    (sR : sumR A A' AR B B' BR s s') :
  sumR A' A (sym_rel AR) B' B (sym_rel BR) s' s :=
  match sR with
  | inlR a a' aR => inlR A' A (sym_rel AR) B' B (sym_rel BR) a' a aR
  | inrR b b' bR => inrR A' A (sym_rel AR) B' B (sym_rel BR) b' b bR
  end.

Definition sumR_flipK {A A' : Type} (AR : A -> A' -> Type)
    {B B' : Type} (BR : B -> B' -> Type) (s : A + B) (s' : A' + B')
    (sR : sumR A A' AR B B' BR s s') :
  sumR_flip (sym_rel AR) (sym_rel BR) s' s (sumR_flip AR BR s s' sR) = sR.
Proof. case sR; constructor. Defined.

Definition sumR_sym {A A' : Type} (AR : A -> A' -> Type)
    {B B' : Type} (BR : B -> B' -> Type) (s : A + B) (s' : A' + B') :
    sumR A A' AR B B' BR s s' <->> sumR A' A (sym_rel AR) B' B (sym_rel BR) s' s.
Proof.
  unshelve econstructor.
  - exact (sumR_flip AR BR s s').
  - unshelve econstructor.
    + exact (sumR_flip (sym_rel AR) (sym_rel BR) s' s).
    + exact (sumR_flipK AR BR s s').
Defined.
...
- exact: Map0_sum.
- constructor.
  exact (Map1.map _ (Map1_sum _ _
    (Param01_sym _ _ AR) _ _ (Param01_sym _ _ BR))).
Defined.

...
% llncs does not define \doi, but splncs04.bst emits it. DOIs routinely
% contain underscores, which arrive from the .bbl already tokenised as
% subscripts; \detokenize re-reads them as printable characters, avoiding
% the "Missing $ inserted" error.
\providecommand{\doi}[1]{\textsc{doi}:\ \texttt{\detokenize{#1}}}
...
\titlerunning{When Does Proof Transfer Pay Off?}
...
constant; only the intercepts differ. Figure~\ref{fig:roi} plots the
saturating return in both regimes.
...
break-even; it does not abolish it. Figure~\ref{fig:sensitivity} plots the
return over both variables at once, with the break-even locus drawn on the
surface. The asymptotic return rises to
...
Figure~\ref{fig:typeclass} quantifies the intermediate case: in
\texttt{bs\_a2.v} the rename still compiles after repair, and the effect is
visible on both sides at once---the per-theorem manual cost rises from $5.67$
to $6.00$ steps while Trocq's setup rises from $53$ to $82$, pushing the
break-even out from $14.5$ to $20.5$ even as the asymptotic ceiling improves
from $1.83\times$ to $2.00\times$.

\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{_graphs/fig5_typeclass}
\caption{What introducing the \texttt{Addable} typeclass costs. (a) Both
sides get dearer: the copy-paste baseline because two steps must be repaired
by hand, Trocq because a fourth function needs its own bridge, witness and
registration. (b) The long-run ceiling rises, but the break-even moves
further out---an improvement in the limit bought with a longer wait.}
\label{fig:typeclass}
\end{figure}
...
(Table~\ref{tab:regimes} and Figure~\ref{fig:regimes}).
Figure~\ref{fig:iterations} traces how the estimate moved across the four
iterations: the first three refine the model, the fourth changes what the
model is measured against, and it is the one that dominates. What decides which curve applies is whether
...
\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{_graphs/fig1_regimes}
\caption{Cumulative transfer cost in the two regimes. The shaded wedge is the
interval in which Trocq has not yet repaid its setup. Both panels share axes;
only the manual baseline differs. Constants are measured, not fitted by eye.}
\label{fig:regimes}
\end{figure}

\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{_graphs/fig6_iterations}
\caption{The break-even estimate across the four iterations. The first three
move it by refining the model; the fourth moves it by changing what the model
is compared against, which is why it dominates.}
...
\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{_graphs/fig2_roi}
\caption{Return on investment against Trocq's own cost. Both curves saturate:
the marginal costs of the two approaches are constant, so only the intercepts
differ, and no amount of volume buys an unbounded return.}
\label{fig:roi}
\end{figure}

\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{_graphs/fig3_roi_surface}
\caption{The return-on-investment surface over both governing variables at
once. The black curve is the break-even locus $\mathrm{ROI}=0$: below it Trocq
has not repaid its setup. Increasing complexity moves that locus toward fewer
theorems, but the surface flattens along $n$, so the return saturates rather
than growing without bound. Drawn for Regime B, where $k$ was fitted; it does
not govern the copy-paste baseline.}
...
Figure~\ref{fig:setup} shows both: the anatomy of the setup, and how
registration count tracks the complexity of the type former rather than the
number of theorems.

\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{_graphs/fig4_setup}
\caption{(a) Where Trocq's setup cost goes; relational witnesses and bridge
lemmas dominate, and the six registrations are the cheapest part. (b)
Registrations required per file. Setup scales with the complexity of the type
former: a function-encoded map needs the relatedness lattice registered by
hand for every participating type.}
\label{fig:setup}
\end{figure}
...
The Coq/Rocq sources, the counting script and the generated measurement
tables are distributed with this paper. Every constant reported above can be
regenerated by re-running that one script over the corpus, and the figures
are plotted from the table it emits, so text and figures cannot drift apart.

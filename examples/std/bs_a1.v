(* ===========================================================================
   bs_a1.v -- ROI experiment with a COPY-PASTE manual baseline
   ---------------------------------------------------------------------------
   Role       : Baby Step 2 - the adversarial baseline (REGIME A)
   Depends on : self-contained
   Provenance : authored for this study
   Measured   : 29 proofs, 138 tactic steps, 9 [Trocq Use]

   Reference file for the counting convention.  All 29 of its per-lemma
   annotations were verified against _count_steps.py and match exactly.
   Its decomposition (per-function = bridge + R__ wrapper + Trocq Use)
   is the accounting scheme used throughout the study.
...
(*  Phase 3 tactic-step summary
    ...
    │  Per theorem ×3                          │      6       │
    ├──────────────────────────────────────────┼──────────────┤
    │  TOTAL (Trocq setup + theorems)          │     59       │
    │  SETUP ONLY (without per-theorem)        │     53       │
    └──────────────────────────────────────────┴──────────────┘

    (Trocq Use commands counted as 1 step each above.)

    CORRECTION (2026-08-10): this table previously reported S_bij = 13 and
    S_setup = 50.  Its own rows sum to 4 + 4 + 5 + 3 = 16, not 13 -- the three
    shared [Trocq Use] commands were listed but not added in, even though the
    note directly below states they count as one step each.  The per-lemma
    counts were and remain correct; only the subtotal was wrong.  Verified
    against _count_steps.py.

    For ROI:  S_setup = 53 (one-time),  n * 2 (per theorem)
    C_trocq(n) = 53 + 2n
*)
...
(* ============================================================
   ...
   Phase 3  (Trocq)
     S_bij  (isos + R_NatList + 3 shared Trocq Use)  :  16 tactic steps
                                                        (4 + 4 + 5 + 3)
     f * (S_bridge + W_trocq)  with f = 3            :  37 tactic steps
       plength: 5 + 6  =  11
       papp:    6 + 8  =  14
       prev:    6 + 6  =  12
     S_setup  = S_bij + f*(S_bridge + W_trocq)        :  53
   ...
   COMPARE with the bridge-based baseline (bs_p4/bs_p5/bs_p6):
     manual = bijection-based, f = 3:         n*  ≈  3.1
     manual = copy-paste (this file):         n*  ≈  14.5

   The break-even moves out by roughly a factor of five.  The
   copy-paste baseline is so cheap that Trocq must amortise its
   53-step setup across at least 15 theorems before it pays off.
   ...
   In our numbers S_bij = 16 includes those 3 commands (4 + 4 + 5 + 3),
   ...
   NOTE: ROI_inf is unaffected by the S_bij correction -- it depends
   only on the marginal costs (P_paste and 2), not on the setup.
*)

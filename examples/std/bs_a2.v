(* ===========================================================================
   bs_a2.v -- polymorphic PList with an Addable typeclass: where copy-paste breaks
   ---------------------------------------------------------------------------
   Role       : Baby Step 2 - the signature-divergence experiment (REGIME A -> B)
   Depends on : self-contained
   Provenance : authored for this study
   Measured   : 40 proofs, 200 tactic steps, 11 [Trocq Use]

   The psum_papp_manual proof is the pivot of the whole study: a literal
   rename of nsum_napp does NOT typecheck, and two steps must be repaired
   with reasoning specific to the Addable signature.
...
(* =============================================================================
   ...
   All counts below are MEASURED by _count_steps.py under the project-wide
   convention (one period-terminated tactic = 1 step; [t1; t2.] = 1 step;
   bullets = 0; [Trocq Use] counted separately as commands).  Do not hand-edit;
   regenerate.  An earlier revision of this table was hand-written and was wrong
   in nearly every entry (it reported a shared subtotal of 20 against a measured
   33, and manual costs of 7/5/5/6 against a measured 5/5/7/7).

   ┌──────────────────────────────────────────────┬─────────────────────────────┐
   │  SHARED BASELINE (NatList theorems; BOTH     │  Tactic Steps               │
   │  approaches need these)                      │                             │
   ├──────────────────────────────────────────────┼─────────────────────────────┤
   │  napp_nil_r                                  │   4                         │
   │  nlength_napp                                │   5                         │
   │  napp_assoc                                  │   5                         │
   ...
   ┌──────────────────────────────────────────────┬─────────────────────────────┐
   │  MANUAL SIDE — polymorphic PList A           │  Tactic Steps               │
   ├──────────────────────────────────────────────┼─────────────────────────────┤
   │  papp_nil_r                       (aux)      │   5                         │
   │  plength_papp_manual                         │   5   copy-paste clean      │
   │  papp_assoc_manual                           │   5   copy-paste clean      │
   │  prev_papp_manual                            │   7   copy-paste clean      │
   │  psum_papp_manual                            │   7   *** DIVERGES ***      │
   ├──────────────────────────────────────────────┼─────────────────────────────┤
   │  MANUAL SUBTOTAL (4 theorems + 1 aux)        │  29                         │
   └──────────────────────────────────────────────┴─────────────────────────────┘

   The three "copy-paste clean" rows are literal renames of their NatList
   counterparts.  psum_papp_manual is NOT: the rename produces a script that
   does not typecheck, and two steps must be repaired by hand with reasoning
   specific to the Addable signature (see DIVERGENCE 1 and DIVERGENCE 2 at the
   proof site).  This is the phenomenon the cost model calls REGIME B.

   ┌──────────────────────────────────────────────┬─────────────────────────────┐
   │  TROCQ SETUP — nat                           │  Tactic Steps               │
   ├──────────────────────────────────────────────┼─────────────────────────────┤
   │  Isomorphism block                           │                             │
   │    plist_nlist_iso / nlist_plist_iso         │   6 + 6                     │
   │    R_NatList / R_NatList_nat                 │   5 + 1                     │
   │    Trocq Use x3 (shared)                     │   3 cmds                    │
   │    S_bij subtotal                            │  21                         │
   │  Bridge lemmas (length, app, rev, sum)       │   5 + 5 + 7 + 6  =  23      │
   │  Generic R_ wrappers (4)  — see note below   │   5 + 7 + 5 + 5  =  22      │
   │  Concrete R_*_nat wrappers (4) + 4 Trocq Use │   6 + 8 + 6 + 8 + 4 = 32    │
   │  psum_nat_eq_psum (alias bridge)             │   6                         │
   ├──────────────────────────────────────────────┼─────────────────────────────┤
   │  TROCQ SETUP SUBTOTAL (nat)                  │ 104                         │
   └──────────────────────────────────────────────┴─────────────────────────────┘

   NOTE on the 22 wasted steps.  The generic wrappers R_plength / R_papp /
   R_prev / R_psum are correct and fully polymorphic, but CANNOT be registered:
   [Trocq Use] requires the conclusion's head to be a concrete global reference,
   and theirs is the lambda-bound f (see the comment at the registration site).
   They had to be duplicated as concrete R_*_nat wrappers.  These 22 steps are
   pure overhead imposed by the tool's gref-keyed database — the honest cost of
   the "monomorphic alias tax", measured.

   ┌──────────────────────────────────────────────┬─────────────────────────────┐
   │  MARGINAL COST OF A SECOND ELEMENT TYPE (Z)  │  Manual      │  Trocq       │
   ├──────────────────────────────────────────────┼──────────────┼──────────────┤
   │  papp_assoc_Z                                │   1          │  (2)         │
   │  prev_papp_Z                                 │   1          │  (2)         │
   │  plength_papp_Z              [see dagger]    │   7          │  (2)         │
   │  New R_*_Z wrappers (3) + 4 Trocq Use        │   0          │  24          │
   ├──────────────────────────────────────────────┼──────────────┼──────────────┤
   │  MARGINAL SUBTOTAL FOR Z                     │   9          │  30          │
   └──────────────────────────────────────────────┴──────────────┴──────────────┘

   [dagger] plength_papp_Z is proved here through the NatList bridge in 7 steps to
   demonstrate that the bridge infrastructure is not nat-specific.  It is NOT
   the cheapest manual route: plength_papp_manual is already polymorphic and
   discharges it in 1 step, exactly like papp_assoc_Z and prev_papp_Z.

   ── 4. Conclusion — corrected ────────────────────────────────────────────────

   The per-theorem economy is real and large: 2 steps against 5-7, a 60-70%
   reduction on each transferred theorem.  Two earlier claims, however, do NOT
   survive measurement and are withdrawn:

   (a) "We avoid manually repeating inductive arguments for different types."
       The opposite holds in this file.  A manual proof that can be STATED
       polymorphically generalises to a new element type for ~1 step
       (papp_assoc_Z, prev_papp_Z).  Trocq needs a fresh concrete wrapper set
       per element type: 30 steps for Z against the manual side's 9.  Trocq
       LOSES the cross-type amortisation argument here.

   (b) "A small one-time cost."  The nat setup measures 104 steps against a
       manual side of 29.  It is not small, and against a copy-paste baseline
       it is not recovered within this file's six theorems.

   What Trocq does buy, and what this file actually demonstrates, is the
   psum row: the one theorem where copy-paste stops being mechanical because
   the algebraic signature diverges.  That is the case the cost model must be
   read against — not the three rows where a rename already worked.
   See bs_z1_lift.v for the sharper version, where the rename does not merely
   need repair but fails outright.
   ============================================================================= *)

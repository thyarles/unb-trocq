(* ===========================================================================
   bs_m1.v -- Trocq transfer over higher-order (function-encoded) maps
   ---------------------------------------------------------------------------
   Role       : Baby Step 2 - the scalability probe: how setup grows with type-former complexity
   Depends on : Stdlib String
   Provenance : authored for this study
   Measured   : 13 proofs, 52 tactic steps, 35 [Trocq Use]

   Requires 35 [Trocq Use] registrations against bs_p5.v's 5, because the
   full relatedness lattice (Param2a0/02b/10/01/00) must be registered by
   hand for each of val, string, Bool, option_nat and nat.  This is the
   measured cost of Trocq on a non-first-order type former.

   ATTRIBUTION: the map interface formalised here -- total_map, tm_empty,
   tm_update, partial_map, pm_empty, pm_update, and the tm_update_neq
   property -- follows _Software Foundations_, Volume 1: Logical Foundations,
   chapter "Maps" (Pierce et al., MIT license).  The definitions are
   re-declared standalone rather than imported, but the design is theirs and
   must be cited as such.  A verbatim reference copy of that chapter was
   previously kept alongside this file as bs_m1_sf_maps.v; it was removed on
   2026-08-10 because nothing imported it and, being third-party, it could
   not count towards the study's effort measurements.

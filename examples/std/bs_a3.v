(* ===========================================================================
   bs_a3.v -- parametric infrastructure fixing the A2 comparison asymmetry
   ---------------------------------------------------------------------------
   Role       : Baby Step 2 - like-for-like manual/Trocq comparison + Z amortization
   Depends on : self-contained
   Provenance : authored for this study

   BUILD STATUS: compiles clean; listed in _CoqProject since 2026-08-10.
   It previously failed to build (missing match motive in map_in_R_PList) and
   was excluded from _CoqProject, so it had never been exercised.

   *** CAVEAT ON PHASE 7 -- read before citing this file ***
   Param_psum and Param_papp (Phase 6) are DEFINED but never APPLIED.  The
   only lemma that consumed them, Param_zsum_zapp, is now discharged by
   [exact (@psum_papp_manual Z addable_Z)] -- i.e. by the MANUAL polymorphic
   proof, not by the parametric Trocq infrastructure.  AddableR_Z_eq and
   PListR_refl are likewise built and then unused.

   This is not an oversight in the repair: the original Route B term was
   ill-typed and could not have worked.  Param_psum concludes
   [AR (psum l) (psum l')] -- psum agrees on two RELATED lists -- whereas the
   goal is [psum (papp l1 l2) = add (psum l1) (psum l2)], an ALGEBRAIC
   distributivity statement.  The right-hand side is not of the form
   [psum l'] for any list l', so no instantiation of Param_psum yields it.

   Consequence: Phase 7 does NOT currently demonstrate amortization of the
   Phase 6 parametric infrastructure to a second element type.  What it
   demonstrates is that a manual polymorphic proof transfers to Z in one
   step.  The Route B rows in the Phase 7 and Phase 8 tables below describe
   code that is not on any proof path and must not be cited as measured
   evidence until Route B is given a statement Param_psum can actually prove.
   Measured   : 34 proofs, 150 tactic steps, 8 [Trocq Use]
...
Definition map_in_R_PList (A A' : Type) (AR : Param2a0.Rel A A') :
    ...
    fun l l' e =>
        match e in (_ = l') return (PListR A A' AR l l') with
        | eq_refl =>
            (fix F (k : PList A) : PListR A A' AR k (map_PList A A' AR k) :=
                match k as k0 return PListR A A' AR k0 (map_PList A A' AR k0) with
                | PNil     => PNilR A A' AR
                | PCons a tl =>
                    PConsR A A' AR
                        a (map AR a) (map_in_R AR a (map AR a) idpath)
                        tl (map_PList A A' AR tl) (F tl)
                end) l
        end.
...
Definition PListR_flip (A A' : Type) (AR : A -> A' -> Type) :
    ...
    fix F l l' lR :=
        match lR with
        | PNilR _ _ _ => PNilR A' A (sym_rel AR)
        | PConsR _ _ _ a a' aR l l' lR =>
...
Arguments zeroR {A A' AR HA HA'}.
Arguments addR {A A' AR HA HA'}.
...
Definition addable_Z : Addable Z := {|
    add        := Z.add;
    zero       := 0%Z;
    add_assoc  := fun x y z => eq_sym (Z.add_assoc x y z);
...
Definition AddableR_Z_eq : AddableR Z Z eq addable_Z addable_Z :=
    Build_AddableR Z Z eq addable_Z addable_Z eq_refl (fun a a' aR b b' bR => f_equal2 Z.add aR bR).
...
Proof.
    exact (@psum_papp_manual Z addable_Z).
Qed.

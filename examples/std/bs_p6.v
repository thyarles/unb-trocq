(* ===========================================================================
   bs_p6.v -- adds list reversal: the complexity benchmark
   ---------------------------------------------------------------------------
   Role       : Baby Step 1 - controlled increase in per-theorem structural complexity
   Depends on : bs_p1, bs_p2, bs_p5
   Provenance : authored for this study
   Measured   : 8 proofs, 46 tactic steps, 1 [Trocq Use]
...
(*  We prove _prev_papp WITHOUT trocq, by manually injecting the
    ...
    Proof obligations incurred (per-theorem cost):
       1. intros l1 l2                 — introduce the variables
       2. rewrite plist_nlist_iso      — inject the isomorphism (LHS)
       3. rewrite plist_nlist_iso      — inject the isomorphism (RHS)
       4. apply f_equal                — reduce to NatList equality
       5. rewrite plist_2_nlist_rev    — bridge: _prev → nrev
       6. rewrite plist_2_nlist_app    — bridge: _papp → napp (LHS)
       7. rewrite nrev_napp            — apply the NatList theorem
       8. rewrite plist_2_nlist_app    — bridge: _papp → napp (RHS)
       9. rewrite plist_2_nlist_rev    — bridge: _prev → nrev
      10. rewrite plist_2_nlist_rev    — bridge: _prev → nrev
      11. reflexivity
      ─────────────────────────────────
      TOTAL: 11 tactic steps

    NOTE (2026-08): this block previously reported 9 steps, obtained by
    grouping the two repeated [rewrite plist_nlist_iso] and the two repeated
    [rewrite plist_2_nlist_rev] into single entries.  Under the project-wide
    convention (one period-terminated tactic = 1 step) the count is 11.
    Regenerate with _count_steps.py rather than editing by hand.            *)
